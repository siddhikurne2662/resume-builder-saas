// src/app/builder/ProductManagerTemplate.tsx
import React from 'react';
import { EnhancedResumeData } from '@/types/resume';

interface ProductManagerTemplateProps {
...
const ProductManagerTemplate: React.FC<ProductManagerTemplateProps> = ({ resumeData }) => {
        ...
        <div key={exp.id || index} className="mb-6">
          <div className="flex justify-between items-start mb-2">
            <div>
              ...
              <div className="text-sm whitespace-pre-line">{exp.title}</div>
            </div>
            <div className="text-right text-sm">
              <div className="font-bold">{exp.address}</div>
              <div className="whitespace-pre-line">{exp.startDate} – {exp.endDate}</div>
            </div>
          </div>
          {/* Assuming a summary property can be added to experience for the description */}
